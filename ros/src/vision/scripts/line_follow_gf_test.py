...
def get_ex_cnts():
    ...
    for img in range(len(circles)):
        ...
        # threshold for blue
        circ_thresh.append(
            cv2.inRange(circles[img], (182 / 2, 20 * 2.56, 20 * 2.56), (225 / 2, 100 * 2.56, 100 * 2.56)))
...
def traverse_line(img_og, contour, vects):
    ...
    cv2.line(img_og, (center[0], center[1]), (center[0] + curr_thrust_vect[0], center[1] + curr_thrust_vect[0]),
             (0, 255, 0), 1)
    cv2.line(img_og, (center[0] + curr_thrust_vect[0], center[1] + curr_thrust_vect[0]),
             (center[0] + curr_thrust_vect[0] + \
...
def process(data):
    ...
    if View.at_beginning:
        ...
        cv2.circle(img_og, (start_point_vector[0], start_point_vector[1]), 3, (0, 0, 255), -1)
        cv2.line(img_og, (start_point_vector[0], start_point_vector[1]), (wall_md_pt[0], wall_md_pt[1]),
                 (150, 255, 255), 1)
        ...
        curr_thrust_vect[0], curr_thrust_vect[1], resultant_vect[0], resultant_vect[1]))
